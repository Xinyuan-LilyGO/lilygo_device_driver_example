/*
 * @Description: 在 T-Display-P4 V2.0 上通过 USB Hub 测试网卡和 U 盘，按 BOOT
 * 安全卸载 U 盘
 * @Author: LILYGO_L
 * @Date: 2026-05-20
 * @LastEditTime: 2026-09-22 17:04:44
 * @License: GPL 3.0
 */
#include <dirent.h>
#include <sys/stat.h>
...
constexpr char kMscRwTestFileName[] = "usb_hub_rw_test.txt";
#endif
...
int FindMscSlotByHandle(msc_host_device_handle_t handle) {
  for (int i = 0; i < kMaxMscDevices; ++i) {
    if (g_msc_devices[i] != nullptr && g_msc_devices[i]->msc_device == handle) {
      return i;
    }
  }
  return -1;
...
void ScanMscFiles(int slot) {
  char mount_path[16] = {};
  snprintf(mount_path, sizeof(mount_path), "%s%d", kMscMountRoot, slot);
...
esp_err_t RunMscRwTestOnce(int slot) {
  ...
  uint8_t* write_buffer = static_cast<uint8_t*>(malloc(kMscRwTestFileSize));
  uint8_t* read_buffer = static_cast<uint8_t*>(malloc(kMscRwTestFileSize));
  ...
  FillTextTestBuffer(write_buffer, kMscRwTestFileSize, slot, counter);
  memset(read_buffer, 0, kMscRwTestFileSize);
  ...
  if (fwrite(write_buffer, 1, kMscRwTestFileSize, file) != kMscRwTestFileSize) {
  ...
  if (fread(read_buffer, 1, kMscRwTestFileSize, file) != kMscRwTestFileSize) {
  ...
  if (memcmp(write_buffer, read_buffer, kMscRwTestFileSize) != 0) {
...
esp_err_t AllocateNewMscDevice(uint8_t usb_addr) {
  ...
  if (entry == nullptr) {
    printf("allocate MSC device entry failed\n");
    return ESP_ERR_NO_MEM;
  }
...
bool FreeMscDevice(int slot) {
  if (slot < 0 || slot >= kMaxMscDevices || g_msc_devices[slot] == nullptr) {
    return true;
  }
...
extern "C" void app_main() {
  ...
  printf(
      "Connect RTL8152B and USB flash drive through the USB hub, then run "
      "iperf\n");
  StartIperfConsole();
