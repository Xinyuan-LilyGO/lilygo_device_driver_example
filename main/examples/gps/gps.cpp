 ...
 * @LastEditTime: 2026-09-22 17:03:47
 * @License: GPL 3.0
 */
#include "gps.h"
...
void PrintUtc(
    const char* source, const char* sentence, const NmeaParser::UtcTime& utc) {
  if (utc.valid) {
...
void PrintDate(
    const char* source, const char* sentence, const NmeaParser::UtcDate& date) {
  if (!date.valid) {
    return;
  }
  if (date.two_digit_year) {
    printf("[%s][%s] UTC date: %02u-%02u-%02u (two-digit NMEA year)\n", source,
        sentence, static_cast<unsigned int>(date.year),
...
void PrintCoordinate(const char* source, const char* sentence, const char* name,
    const NmeaParser::Coordinate& coordinate) {
  if (!coordinate.valid) {
    return;
  }
  printf("[%s][%s] %s: %.10lf deg (%u deg %.7lf min %c)\n", source, sentence,
      name, coordinate.decimal_degrees,
...
void PrintFloat(const char* source, const char* sentence, const char* name,
  ...
  if (field.valid) {
    printf("[%s][%s] %s: %.3f%s\n", source, sentence, name, field.value, unit);
  }
}
...
void PrintCharacter(const char* source, const char* sentence, const char* name,
    const NmeaParser::OptionalValue<char>& field) {
  if (field.valid) {
...
void PrintString(const char* source, const char* sentence, const char* name,
  ...
  if (field.valid) {
    printf("[%s][%s] %s: %s\n", source, sentence, name, field.value.c_str());
  }
}
...
void PrintDtm(const char* source, const NmeaParser::Dtm& value) {
  ...
  printf("[%s][DTM] local datum: %s, subdivision: %s, reference: %s\n", source,
      value.local_datum.c_str(), value.local_datum_subdivision.c_str(),
      value.reference_datum.c_str());
  PrintDouble(
      source, "DTM", "latitude offset", value.latitude_offset_minutes, " min");
  PrintCharacter(source, "DTM", "latitude direction", value.latitude_direction);
  PrintDouble(source, "DTM", "longitude offset", value.longitude_offset_minutes,
      " min");
  PrintCharacter(
      source, "DTM", "longitude direction", value.longitude_direction);
  PrintDouble(
      source, "DTM", "altitude offset", value.altitude_offset_meters, " m");
}
...
void PrintGbs(const char* source, const NmeaParser::Gbs& value) {
  ...
  PrintUnsigned(source, "GBS", "failed satellite", value.failed_satellite_id);
  PrintDouble(source, "GBS", "missed detection probability",
      value.missed_detection_probability);
  PrintDouble(
      source, "GBS", "estimated bias", value.estimated_bias_meters, " m");
...
void PrintGga(const char* source, const NmeaParser::Gga& value) {
  ...
  PrintDouble(
      source, "GGA", "geoid separation", value.geoid_separation_meters, " m");
  PrintDouble(
      source, "GGA", "differential age", value.differential_age_seconds, " s");
  PrintString(
      source, "GGA", "differential station ID", value.differential_station_id);
}
...
void PrintGll(const char* source, const NmeaParser::Gll& value) {
  ...
  PrintCharacter(source, "GLL", "positioning mode", value.positioning_mode);
}
...
void PrintGns(const char* source, const NmeaParser::Gns& value) {
  ...
  PrintDouble(
      source, "GNS", "geoid separation", value.geoid_separation_meters, " m");
  PrintDouble(
      source, "GNS", "differential age", value.differential_age_seconds, " s");
  PrintString(
      source, "GNS", "differential station ID", value.differential_station_id);
  PrintCharacter(
      source, "GNS", "navigational status", value.navigational_status);
}
...
void PrintGsv(const char* source, const NmeaParser::Gsv& value) {
  ...
  printf("[%s][GSV] talker: %s, messages: %u, satellites in view: %u", source,
  ...
  for (const auto& satellite : value.satellites) {
    ...
    if (satellite.carrier_to_noise_db_hz.valid) {
      printf(", C/N0 %u dB-Hz",
          static_cast<unsigned int>(satellite.carrier_to_noise_db_hz.value));
    }
    printf("\n");
  }
}
...
void PrintRlm(const char* source, const NmeaParser::Rlm& value) {
  ...
  printf("[%s][RLM] message body: %s\n", source, value.message_body.c_str());
}
...
void PrintRmc(const char* source, const NmeaParser::Rmc& value) {
  ...
  PrintFloat(
      source, "RMC", "speed over ground", value.speed_over_ground_knots, " kn");
  ...
  PrintCharacter(source, "RMC", "positioning mode", value.positioning_mode);
  PrintCharacter(
      source, "RMC", "navigational status", value.navigational_status);
}
...
void PrintTxt(const char* source, const NmeaParser::Update& update,
  ...
  for (size_t index = 0; index < value.messages.size(); ++index) {
    printf("[%s][TXT] %u/%u", source, static_cast<unsigned int>(index + 1),
        static_cast<unsigned int>(value.total_sentence_count));
...
void PrintVtg(const char* source, const NmeaParser::Vtg& value) {
  ...
  PrintFloat(source, "VTG", "true course", value.true_course_degrees, " deg");
  PrintFloat(
      source, "VTG", "magnetic course", value.magnetic_course_degrees, " deg");
  PrintFloat(source, "VTG", "speed", value.speed_knots, " kn");
  PrintFloat(source, "VTG", "speed", value.speed_kilometers_per_hour, " km/h");
  PrintCharacter(source, "VTG", "positioning mode", value.positioning_mode);
}
...
void PrintPubxPosition(
  ...
  PrintString(source, "PUBX,00", "navigation status", value.navigation_status);
  ...
  PrintFloat(
      source, "PUBX,00", "speed", value.speed_kilometers_per_hour, " km/h");
  ...
  PrintUnsigned(source, "PUBX,00", "satellites used", value.satellites_used);
...
void PrintPubxSatelliteStatus(
  ...
  for (const auto& satellite : value.satellites) {
    ...
    if (satellite.carrier_lock_time_seconds.valid) {
      printf(", lock %u s",
          static_cast<unsigned int>(satellite.carrier_lock_time_seconds.value));
    }
    printf("\n");
  }
}
...
void PrintPubxTime(const char* source, const NmeaParser::PubxTime& value) {
  ...
  PrintUnsigned(source, "PUBX,04", "UTC week", value.utc_week_number);
  ...
  if (value.clock_bias_nanoseconds.valid) {
    printf("[%s][PUBX,04] clock bias: %" PRId64 " ns\n", source,
        static_cast<int64_t>(value.clock_bias_nanoseconds.value));
  }
  ...
  if (value.time_pulse_granularity_nanoseconds.valid) {
    printf("[%s][PUBX,04] time pulse granularity: %" PRId32 " ns\n", source,
        static_cast<int32_t>(value.time_pulse_granularity_nanoseconds.value));
  }
}
...
void PrintNmeaDiagnostics(
  ...
  printf(
      "[%s][NMEA] parsed: %u, checksum errors: %u, format errors: %u, "
