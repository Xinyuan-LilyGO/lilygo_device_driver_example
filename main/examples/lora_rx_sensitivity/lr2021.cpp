 ...
 * @LastEditTime: 2026-09-22 17:04:05
 * @License: GPL 3.0
 */
#include <array>
#include <cstdint>
...
constexpr lr20xx_radio_lora_bw_t kLoraBandwidth =
    kUseHighFrequencyPath ? LR20XX_RADIO_LORA_BW_500 : LR20XX_RADIO_LORA_BW_125;
...
constexpr lr20xx_system_irq_mask_t kRadioIrqMask =
    LR20XX_SYSTEM_IRQ_RX_DONE | LR20XX_SYSTEM_IRQ_LORA_HEADER_ERROR |
...
bool SetPacketConfig(usp_cpp_bus_driver::Lr20xx& lr2021) {
  const lr20xx_radio_lora_pkt_params_t packet_config = MakePacketConfig();
...
void RunLr2021() {
  ...
  while (true) {
    ...
    if (RadioIrqAsserted()) {
      ...
      if (!ReadAndClearIrq(lr2021, irq_status)) {
      ...
      } else if ((irq_status & LR20XX_SYSTEM_IRQ_LORA_HEADER_ERROR) != 0) {
        ...
        session.RecordPacketError(PacketError::kLength, current_time);
