 ...
 * @LastEditTime: 2026-09-22 17:04:01
 * @License: GPL 3.0
 */
...
void TestSession::PrintConfiguration() const {
  const uint32_t bandwidth_khz =
      (kUseHighFrequencyPath && std::strcmp(radio_name_, "LR1121") == 0) ? 406UL
      : (kUseHighFrequencyPath && std::strcmp(radio_name_, "LR2021") == 0)
          ? 500UL
          : static_cast<uint32_t>(kBandwidthKhz);
  printf("Frequency: %" PRIu32 " Hz\n", static_cast<uint32_t>(frequency_hz_));
  printf("Modulation: LoRa, SF%u, BW %" PRIu32 " kHz, CR 4/5\n",
  ...
  printf("Expected RF generator packet count: %" PRIu32 "\n",
      static_cast<uint32_t>(kExpectedPacketCount));
...
void TestSession::PrintSetup() const {
  ...
  printf("Receiver is ready. Transmit exactly %" PRIu32 " packets at each\n",
      static_cast<uint32_t>(kExpectedPacketCount));
...
void TestSession::Start(uint32_t current_time_ms) {
  ...
  printf("[PER] expecting exactly %" PRIu32 " packets\n",
      static_cast<uint32_t>(kExpectedPacketCount));
}

void TestSession::Finish(uint32_t current_time_ms, const char* reason) {
  if (!running_) {
    return;
  }
  running_ = false;
  if (std::strcmp(reason, "expected packet events reached") == 0)
    locked_ = true;
  last_progress_time_ms_ = current_time_ms;
  progress_timer_started_ = true;
  ...
  const uint32_t bounded_observed = std::min(observed, kExpectedPacketCount);
  const uint32_t missing_packets = kExpectedPacketCount - bounded_observed;
  const uint32_t bounded_good = std::min(good_packets_, kExpectedPacketCount);
  const uint32_t failed_packets = kExpectedPacketCount - bounded_good;
  const double per_percent = static_cast<double>(failed_packets) * 100.0 /
  ...
  printf("Elapsed: %" PRIu32 " ms\n", static_cast<uint32_t>(elapsed_ms));
  printf("Expected packets: %" PRIu32 "\n",
      static_cast<uint32_t>(kExpectedPacketCount));
  printf("Correct packets: %" PRIu32 "\n", static_cast<uint32_t>(bounded_good));
  printf("Observed bad packets: %" PRIu32 "\n",
      static_cast<uint32_t>(std::min(observed_bad, kExpectedPacketCount)));
  printf("Unobserved/missing packets: %" PRIu32 "\n",
      static_cast<uint32_t>(missing_packets));
  printf("Header errors: %" PRIu32 "\n", static_cast<uint32_t>(header_errors_));
  printf("CRC errors: %" PRIu32 "\n", static_cast<uint32_t>(crc_errors_));
  printf("Length errors: %" PRIu32 "\n", static_cast<uint32_t>(length_errors_));
  printf("Payload content errors: %" PRIu32 "\n",
      static_cast<uint32_t>(payload_errors_));
  printf(
      "Packet read errors: %" PRIu32 "\n", static_cast<uint32_t>(read_errors_));
  printf("Driver/transport errors: %" PRIu32 "\n",
      static_cast<uint32_t>(driver_errors_));
  printf(
      "Failed packets: %" PRIu32 "\n", static_cast<uint32_t>(failed_packets));
  printf("PER: %.3f%%\n", per_percent);
  ...
  if (packet_rssi_.count > 0) {
    printf("Packet RSSI min/avg/max: %.2f / %.2f / %.2f dBm\n",
        packet_rssi_.minimum, packet_rssi_.Average(), packet_rssi_.maximum);
  }
  if (signal_rssi_.count > 0) {
    printf("Signal RSSI min/avg/max: %.2f / %.2f / %.2f dBm\n",
        signal_rssi_.minimum, signal_rssi_.Average(), signal_rssi_.maximum);
  }
  if (snr_.count > 0) {
    printf("SNR min/avg/max: %.2f / %.2f / %.2f dB\n", snr_.minimum,
        snr_.Average(), snr_.maximum);
  }
...
void TestSession::Poll(uint32_t current_time_ms) {
  ...
  if ((current_time_ms - last_activity_time_ms_) >= kIdleFinishTimeMs) {
    Finish(current_time_ms, "RF packet stream ended");
    return;
  }

  if ((current_time_ms - last_progress_time_ms_) >= kProgressPrintIntervalMs) {
...
void TestSession::Restart(uint32_t current_time_ms) {
  ...
  printf("[PER] waiting for a new %" PRIu32 "-packet RF stream\n",
      static_cast<uint32_t>(kExpectedPacketCount));
}

void TestSession::RecordPacket(const uint8_t* payload, size_t payload_length,
  ...
  if (payload == nullptr || payload_length != kExpectedPayload.size()) {
    ++payload_errors_;
  } else if (std::memcmp(payload, kExpectedPayload.data(),
                 kExpectedPayload.size()) != 0) {
    ++payload_errors_;
  } else {
    ++good_packets_;
...
void TestSession::RecordPacketError(
  ...
  if (observed <= 10) {
    printf("[PER] observed %s error, packet event %" PRIu32 "/%" PRIu32 "\n",
        PacketErrorToString(error), static_cast<uint32_t>(observed),
        static_cast<uint32_t>(kExpectedPacketCount));
  }
  CheckForCompletion(current_time_ms);
}
...
void TestSession::PrintProgress(uint32_t current_time_ms) const {
  ...
  const uint32_t elapsed_ms = current_time_ms - start_time_ms_;
  const double completion_percent = static_cast<double>(observed) * 100.0 /
                                    static_cast<double>(kExpectedPacketCount);
  ...
  printf("Expected packets: %" PRIu32 "\n",
      static_cast<uint32_t>(kExpectedPacketCount));
  printf("Observed packets: %" PRIu32 "\n", static_cast<uint32_t>(observed));
  printf("Remaining to expected count: %" PRIu32 "\n",
      static_cast<uint32_t>(remaining_packets));
  printf(
      "Correct packets: %" PRIu32 "\n", static_cast<uint32_t>(good_packets_));
  printf("Observed bad packets: %" PRIu32 "\n",
      static_cast<uint32_t>(observed_bad));
  printf("Header errors: %" PRIu32 "\n", static_cast<uint32_t>(header_errors_));
  printf("CRC errors: %" PRIu32 "\n", static_cast<uint32_t>(crc_errors_));
  printf("Length errors: %" PRIu32 "\n", static_cast<uint32_t>(length_errors_));
  printf("Payload content errors: %" PRIu32 "\n",
...
uint32_t TestSession::ObservedBadPackets() const {
  return header_errors_ + crc_errors_ + length_errors_ + payload_errors_ +
         read_errors_;
}
